...
public class AStarTest {
	...
	@Test
	public void test6() {
		System.out.println("test6: testing eliminating intermediate elevator/stair cells");
		HashMap<Point, TILE_TYPE> hm1 = new HashMap<Point, TILE_TYPE>();
		for(int i =0; i<100;i++){
			for(int j = 0; j < 100; j++){
				hm1.put(new Point(i,j), TILE_TYPE.ELEVATOR);
		...
		AStar test = new AStar(pcs, "pc2", new AStarConfigOptions());
		ArrayList<CellPoint> AStarOut = test.getPath(new CellPoint("pc3", new Point(50, 50)),
				new CellPoint("pc1", new Point(55, 40)), false);
		if (AStarOut.size() >= 5) {
			System.out.println("getPath didnt break");
			for (CellPoint each : AStarOut) {
